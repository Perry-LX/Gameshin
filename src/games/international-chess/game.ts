// @ts-nocheck
// Full React-compatible chess engine conversion
// Original chess logic preserved completely

type ChessControlElements = {
  speedAsap?: HTMLInputElement | null
  speedFast?: HTMLInputElement | null
  speedMedium?: HTMLInputElement | null
  speedSlow?: HTMLInputElement | null
  randomBlack?: HTMLInputElement | null
  randomWhite?: HTMLInputElement | null
  perspectiveBlack?: HTMLInputElement | null
  perspectiveWhite?: HTMLInputElement | null
}

type ChessGameEndResult = {
  winner: "WHITE" | "BLACK" | null
  stalemate: boolean
}

type InitChessGameOptions = {
  boardElement: HTMLElement
  viewElement: HTMLElement
  controls: ChessControlElements
  onGameEnd?: (result: ChessGameEndResult) => void
}

export function initChessGame({ boardElement, viewElement, controls, onGameEnd }: InitChessGameOptions) {
  const board = boardElement

  if (!board || !viewElement) return;

  // Prevent duplicate mounting
  board.innerHTML = '';
  viewElement.querySelector('[data-chess-sprite="true"]')?.remove()
  viewElement.insertAdjacentHTML(
    'beforeend',
    `<svg data-chess-sprite="true" aria-hidden style="display: none;" width="170" height="170" viewBox="0 0 170 170" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g id="pawn">
...
class Shape {
  ...
  static shape(player, piece) {
    const glyph = Shape.glyphs[player]?.[piece] || "?"
    return `<svg class="${player} chess-glyph" width="170" height="170" viewBox="0 0 170 170" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <text x="85" y="92" text-anchor="middle" dominant-baseline="middle">${glyph}</text>
    </svg>`
  }
...
class Game {
  ...
  constructor(pieces, piecePositions, turn = "WHITE", onGameEnd = null) {
    this.turn = turn
...
class View {
  constructor(element, game, perspective, rootElement) {
  ...
  handleTileClick(location) {
    const result = this.game.activate(location)
...
class Control {
  inputSpeedAsap = null
  inputSpeedFast = null
  inputSpeedMedium = null
  inputSpeedSlow = null
  inputRandomBlack = null
  inputRandomWhite = null
  inputPerspectiveBlack = null
  inputPerspectiveWhite = null
  autoplayTimeoutId = 0
  boundAutoplay = this.autoplay.bind(this)
  boundUpdateViewPerspective = this.updateViewPerspective.bind(this)

  constructor(game, view, elements) {
    this.game = game
    this.view = view
    this.inputSpeedAsap = elements.speedAsap || null
    this.inputSpeedFast = elements.speedFast || null
    this.inputSpeedMedium = elements.speedMedium || null
    this.inputSpeedSlow = elements.speedSlow || null
    this.inputRandomBlack = elements.randomBlack || null
    this.inputRandomWhite = elements.randomWhite || null
    this.inputPerspectiveBlack = elements.perspectiveBlack || null
    this.inputPerspectiveWhite = elements.perspectiveWhite || null
    this.inputPerspectiveBlack?.addEventListener(
      "change",
      this.boundUpdateViewPerspective
    )
  ...
  destroy() {
    ...
    this.inputPerspectiveBlack?.removeEventListener(
      "change",
      this.boundUpdateViewPerspective
    )
    this.inputPerspectiveWhite?.removeEventListener(
      "change",
      this.boundUpdateViewPerspective
    )
  }
  ...
  scheduleAutoplay() {
    ...
    this.autoplayTimeoutId = window.setTimeout(this.boundAutoplay, this.speed)
  }
...
const view = new View(board, game, perspective, viewElement)
const control = new Control(game, view, controls)
